...
- **Recovery password message è tier-aware**: per tier Pro (con encrypted_mappings server-side cifrati zero-knowledge), il reset password elimina i mapping cifrati e il messaggio post-reset lo dichiara esplicitamente con count. Per tier free (mapping solo in IndexedDB browser, niente server-side encrypted_mappings), il messaggio dichiara correttamente che i mapping del browser non sono toccati e non c'erano mapping cloud da eliminare. Pattern di disclosure tier-aware applicato dopo bug osservato SID-20260526-011753 (claim incondizionale "tutti i mapping eliminati" era falso per tier free).
